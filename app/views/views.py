...
def admin():
    if 'user_id' in session:
        ...
        if is_admin == 1:
            ...
            completed_orders = Order.query.filter(Order.status != 'pending').all()
            
            return render_template("admin.html", PDO=pending_delivery_orders,PPO = pending_pickup_orders, CO=completed_orders)
